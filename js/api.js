...
function rmSelect(list) {
    list.forEach(item => {
        const li = document.getElementById(item.entity_id);
        if (li.classList.contains('select')) {
...
function displayInfoOnPage(notice) {
    ...
    natElement.textContent = `Nationalité: ${get_nat(notice.nationalities[0])}`;
...
// liste des nationalités av leurs codes
        var nationalities = 
